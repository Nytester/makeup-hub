...
export default function CartPageContent() {
  ...
  return (
    <div style={{ padding: '40px 64px 80px', background: '#FAF7F4', minHeight: '70vh' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 48, alignItems: 'start' }}>
        <div>
          {items.map(item => (
            <div key={item.id} style={{ display: 'flex', gap: 20, padding: '20px 0', borderBottom: '1px solid #DDD0C4' }}>
              <div style={{ width: 80, height: 80, background: item.imgBg || '#F0EAE3', borderRadius: 4, flexShrink: 0, border: '1px solid #DDD0C4', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width={28} height={28} viewBox="0 0 24 24" fill="none" stroke="#C4B4A6" strokeWidth={1}>
        ...
        <div style={{ background: '#FDFAF7', border: '1px solid #DDD0C4', borderRadius: 4, padding: '28px 24px' }}>
          ...
          <button style={{ width: '100%', background: '#C4614A', color: '#FDFAF7', border: 'none', borderRadius: 2, padding: 14, fontFamily: 'var(--font-body)', fontSize: 14, fontWeight: 500, cursor: 'pointer', marginBottom: 10 }}>Checkout</button>
          <Link href="/shop" style={{ display: 'block', width: '100%', background: 'transparent', color: '#2C2520', border: '1.5px solid #2C2520', borderRadius: 2, padding: 12, fontFamily: 'var(--font-body)', fontSize: 14, cursor: 'pointer', textDecoration: 'none', textAlign: 'center' }}>Continue shopping</Link>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: '#A89080', textAlign: 'center', marginTop: 14 }}>Free returns · Secure checkout</p>
        </div>
      </div>
    </div>
  )
